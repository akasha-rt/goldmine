            ...
            {foreach from=$items key=k item=data}
                ...
                {if $k eq 'data'}
                    {foreach from=$data key=k item=recordsArray}
                        <tr>
                            {foreach from=$recordsArray key=k item=tableHeader}
                                <th>{$tableHeader.label}</th>
                            {/foreach}
                        </tr>
                            {break}
                    {/foreach}
                    ...
                    {foreach from=$data key=k item=recordsArray}
                        <tr>
                            {foreach from=$recordsArray key=k item=records}
                                {if $records.type eq 'datetimecombo' || $records.type eq 'datetime'}
                                    <td>{$records.value|date_format:'%Y-%m-%d %H:%M:%S'}</td>
                                {else}
                                    <td>{$records.value}</td>
                                {/if}
                            {/foreach}
