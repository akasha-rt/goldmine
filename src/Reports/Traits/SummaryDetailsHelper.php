...
trait SummaryDetailsHelper
{
    ...
    protected function getSummaryNextRow(\Report $report)
    {
        ...
        $nextRow = $report->get_next_row('summary_result', 'summary_columns', false, false, 'widgetReportForSideCar');
    ...
    private function createColumnRow(\Report $report): array
    {
        ...
        for ($j = 0; $j < $report->current_summary_row_count; $j++) {
            $columnRow = $report->get_next_row('result', 'display_columns', false, false, 'widgetReportForSideCar');
     ...
     * @return mixed
     */
    protected function generateCount(&$data)
    {
        if (!is_array($data)) {
            return;
        }
