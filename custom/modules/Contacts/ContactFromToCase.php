    ...
    function data_saving_to_cases($bean, $event, $arguments)
    {
        ...
        if (!empty($bean->id) && (!empty($bean->case_comments_c)|| !empty($bean->reason_for_contact_c)) && $bean->modified_user_id == '38ee82fe-9446-4b97-9ed1-3ae0c10f3fd6') {
            ...
            if ($bean->load_relationship('cases')) {
                ...
                if(!empty($cases)) {
                    foreach($cases as $case) {
                        if($case->description == $bean->case_comments_c && $case->reason_for_contact_c == $bean->reason_for_contact_c && date("Y-m-d", strtotime($case->date_entered)) == date("Y-m-d")) {
                            $found = true;
                        }
                    }
                }
            ...
            if($found == false) {
                ...
                $case_bean->type = $bean->reason_for_contact_c;
                $case_bean->reason_for_contact_c = $bean->reason_for_contact_c;
                $case_bean->save();
                $bean->cases->add($case_bean);
                $sql = "UPDATE contacts_cstm SET reason_for_contact_c = '', case_comments_c = '' WHERE id_c='" . $bean->id . "'";
                $db->query($sql);
            }
        }
    }
