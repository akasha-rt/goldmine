...
class GetQuotePDFtoo extends SugarApi{
    public function registerApiRest(){
        return array(
            'GetQuotePDFtoo' => array(
                'reqType' => 'GET',
                'noLoginRequired' => false,
                'rawReply' => true,
                'allowDownloadCookie' => true,
                'path' => array('getquotepdftoo'),
    ...
    public function MyGetQuotePDFtoo($api, $args){
        ...
        $quote_data = array(
            ...
            'shipping_address_street' => $quote->shipping_address_street,
            'shipping_address_city' => $quote->shipping_address_city,
            'shipping_address_state' => $quote->shipping_address_state,
            'shipping_address_postalcode' => $quote->shipping_address_postalcode,
            'shipping_address_country' => $quote->shipping_address_country,
        ...
        foreach($related_billing_contacts as $key=>$related_billing_contact){
            $contact = array(
                ...
                'end_user_company_c' => $related_billing_contact->end_user_company_c,
        ...
        foreach($related_shipping_contacts as $key=>$related_shipping_contact){
            $contact_shipping = array(
                'first_name' => $related_shipping_contact->first_name,
                'last_name' => $related_shipping_contact->last_name,
                'phone_mobile' => $related_shipping_contact->phone_mobile,
                'phone_work' => $related_shipping_contact->phone_work,
                'email' => $related_shipping_contact->email1,
                'end_user_company_c' => $related_shipping_contact->end_user_company_c,
                'id' => $related_shipping_contact->id
            );
        }
        $return_data['contact_billing'] = $contact;
        $return_data['contact_shipping'] = $contact_shipping;
