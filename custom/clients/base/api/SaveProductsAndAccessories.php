...
class SaveProductsAndAccessories extends SugarApi{
    ...
    public function MySaveProductsAndAccessories($api, $args){
        global $db; 
        $this->requireArgs($args, array('shipping_contact_id','billing_contact_id','shipping_account_id','billing_account_id','product_template_id','name','date_quote_expected_closed','accessories','product_category_id','saved_by_portal_user_id','selectedProductQuantity'));
        ...
        $db->query("UPDATE quotes SET new_sub='{$total}', new_sub_usdollar='{$total}', total='{$total}', total_usdollar='{$total}', subtotal_usdollar='{$total}', subtotal='{$total}', quote_number_portal='{$this->get_quote_number($quote_id)}' WHERE id='{$quote_id}'");
