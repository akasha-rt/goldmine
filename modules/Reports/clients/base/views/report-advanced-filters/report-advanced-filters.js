 ...
 */
 ({
    ...
    _initProperties: function() {
        ...
        this._flattenFilters(elementId, filtersData.Filter_1, this._flattenedFilters, 0, 5);
     ...
     * @return {number}
     */
    _flattenFilters: function(elementId, filterDefs, flattenedFilters, row, column) {
        ...
        if (filterDefs.operator) {
            ...
            _.each(filterDefs, function flatten(subFilterDefs, key) {
                ...
                row = this._flattenFilters(subElementId, subFilterDefs, flattenedFilters, row, column);
            ...
            flattenedFilters[elementId] = {
                type: 'condition',
                value: this.getSummaryText(filterDefs),
     ...
     * @return {string}
     */
    getSummaryText: function(filterData) {
        ...
        const fieldLabel = app.lang.get(runtimeFilterWidget._targetField.vname, runtimeFilterWidget._targetModule);

        let prefixEl = document.createElement('b');
        prefixEl.innerText = fieldLabel;

        let prefix = prefixEl.outerHTML;

        const text = runtimeFilterWidget.getSummaryText();
