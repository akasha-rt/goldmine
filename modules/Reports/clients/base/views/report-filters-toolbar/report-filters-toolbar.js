...
({
    ...
    panelMinimized: function(minimized) {
        this._super('panelMinimized', [minimized]);

        if (minimized) {
            this.$('[data-panelaction="toggleAdvancedFilters"]').hide();
            this.$('.report-filters-toolbar').hide();
        } else {
            this.$('[data-panelaction="toggleAdvancedFilters"]').show();
            this.$('.report-filters-toolbar').show();
        }
    },
