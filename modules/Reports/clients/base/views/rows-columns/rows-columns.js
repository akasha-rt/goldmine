...
({
    ...
    _render: function() {
        ...
        if (this.context.get('rebuildData') && !_.isUndefined(this.data)) {
    ...
    _initializeOrderBy: function() {
        ...
        if (_.isUndefined(this.orderBy)) {
            this.orderBy = {
                field: dataOrderBy ? dataOrderBy.name : _.first(this.data.header).name,
                direction: dataOrderBy ? (dataOrderBy.sort_dir === 'a' ? 'asc' : 'desc') : 'asc',
                table_key: '',
                column_function: '',
                group_function: '',
            };
    ...
    _initializeCustomOrderBy: function(dataLoaded) {
        ...
        if (dataLoaded && lastOrderBy && _.has(this.data, 'orderBy')) {
            orderBy = this._createOrderByMeta(this.data.orderBy, dataLoaded);
        } else if (lastOrderBy) {
    ...
    _createOrderByMeta: function(ordersMeta, dataLoaded) {
        ...
        if (dataLoaded && (!hasName) && (!hasField)) {
            ...
            orderBy.field = !hasName ? orderByMeta.field : orderByMeta.name;
        }
    ...
    _setupPreviewReportPanel: function() {
        ...
        this.context.trigger('report:data:table:loaded', this.collection);
    },
    ...
    _loadReportData: function(options) {
        ...
        const sort = _.isUndefined(this.data) ? null : app.user.lastState.get(self.orderByLastStateKey);
    ...
    _toggleEmptyPanel: function(show) {
        ...
        if (show) {
            ...
            this.$el.find('.dataTable').addClass('hidden');
            this.$el.addClass('noBorderBottom');
        } else {
            emptyPanelEl.addClass('hidden');
            this.$el.find('.dataTable').removeClass('hidden');
