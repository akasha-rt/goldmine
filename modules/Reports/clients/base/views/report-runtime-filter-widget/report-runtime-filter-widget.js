...
({
    ...
    _updateBetweenDateTimes: function(datesMeta, useTime) {
        ...
        if (before && this._isValidDatePickerFormat(before)) {
            this._filterData[datesMeta.beforeDateKey] = this._toFilterDate(before, useTime);
    ...
    enumMultipleChanged: function(e) {
        ...
        if (insert) {
            ...
            if (this._filterData.input_name0.length === 1) {
                app.alert.show('runtime-filter-limitation', {
                    level: 'warning',
                    messages: app.lang.get('LBL_RUNTIME_FILTER_LIMITATION'),
                    autoClose: true
                });
    ...
    _getTooltipText: function() {
        ...
        const tableHierarchy = targetTable.name ? targetTable.name : targetTable.value;
        ...
        const title = '<div class="runtime-filter-summary-tooltip"><b>' +
    ...
    _getTargetField: function() {
        ...
        if (targetField.type === 'enum' && targetField.function) {
            ...
            app.api.call('create', url, {
                function: targetField.function,
                functionBean: targetField.function_bean,
            }, {
     ...
     * @return {string}
     */
    _toFilterDate: function(userDate, useTime) {
        const userDatePref = app.user.getPreference('datepref').toUpperCase();
        const unformattedDateTime = `${userDate} ${this._filterData.input_name1}`;
