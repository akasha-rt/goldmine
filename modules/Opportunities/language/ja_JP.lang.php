...
// BEGIN ENT/ULT
    ...
    // List View Help Text
    'LBL_HELP_RECORDS' => '{{plural_module_name}} モジュールを使用すると、個々の売上を最初から最後まで追跡できます。各 {{module_name}} レコードは販売見込みを表し、関連する販売データだけでなく、 {{quotes_module}} や {{contacts_module}} などの他の重要なレコードに関連するデータも含まれます。{{module_name}} は、通常、いくつかの販売段階を経て、「クローズされた受注」または「クローズされた失注」とマークされます。 {{plural_module_name}} は、Sugarの {{forecasts_singular_module}}ing モジュールを使うことでさらに活用でき、販売傾向を理解および予測し、販売ノルマを達成するために作業を集中させることができます。',
