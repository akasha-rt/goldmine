...
({
    ...
    setActiveMetric: function(metric) {
        app.user.lastState.set(this.getActiveMetricKey(), metric.id);
     ...
     * @param {Object} metric The metric that is being hidden
     */
    ...
    updateMetricsCounts: function() {
        ...
        $.each(this.context.metrics, function(i, metric) {
            if (metric.module || metricModule) {
                ...
                app.api.call('read', url, null, {
                    success: _.bind(function(data) {
                        $('#ribbon-' + metric.id).find('.kpi-metric-number-row').text(data.record_count);
                    }, this),
                });
            }
        }, this);
    ...
    getNextActiveMetric: function(model) {
        ...
        // if current metric is the last metric in the list then return
        if (metricsLen === 1) {
            return nextModel;
        }

        const currentIndex = _.findIndex(this.context.metrics, (metric) => metric.id === model.id);
    ...
    _resetRibbon: function() {
        ...
        if (this.context && this.context.metrics && this.context.metrics.length) {
            ...
            if (!app.user.lastState.get(activeKey)) {
                app.user.lastState.set(activeKey, this.context.metrics[0].id);
            }
            ...
            let activMetric = app.user.lastState.get(activeKey);

            if (activMetric && this.$('#ribbon-' + activMetric).is(':hidden')) {
                this.replaceMetric(activMetric);
            }
        }
    },
    ...
    _addTab: function(metric) {
        ...
        if (metric.id == app.user.lastState.get(this.getActiveMetricKey())) {
    ...
    toggleResize: function(resize) {
        this.context.off('view:resize');

        if (resize) {
            this.context.parent.on('view:resize', this.resize, this);
        }
    },
    ...
    _dispose: function() {
        if (this.context && this.context.parent) {
            this.context.parent.off('view:resize');
        }
     ...
     */
    resize: function() {
        let width = $(window).width() - 24; // minus kebab width
        ...
        $dropdownMenu.css('left', $metricList.width() - $dropdownMenu.width());
    },
    ...
    /**
     * @param {string} metric The metric you want to turn on/off.
     * @param {boolean} state `true` to show it on mega menu, `false`
     *   otherwise. If no state given, will toggle.
     * @return {Object}
     */
    toggleMetric: function(metric, state) {
        if (!this._catalog[metric].short) {
            state = !_.isUndefined(state) ? !state : undefined;
            this._catalog[metric].long.toggleClass('hidden', state);
            return this;
        }

        // keep it in sync
        let newState = this._catalog[metric].short.toggleClass('hidden', state).hasClass('hidden');
        this._catalog[metric].long.toggleClass('hidden', !newState);
    ...
    replaceMetric: function(metric) {
        ...
        this.$('#ribbon-' + metric).toggleClass('hidden', false);
        this.$('#overflow-' + metric).toggleClass('hidden', true);
    },
    ...
    selectDropdownMetricItem: function(event) {
        let metric = event.target.closest('li').dataset.metric;
        app.user.lastState.set(this.getActiveMetricKey(), metric);
        this.model.set('active', metric);
    },
    ...
    getActiveMetricId: function() {
        return app.user.lastState.get(this.getActiveMetricKey());
    },
})
