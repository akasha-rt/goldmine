...
use Sugarcrm\Sugarcrm\custom\Hint\Queue\Event\UserInactiveEvent;
use Sugarcrm\Sugarcrm\custom\Hint\Queue\Event\UserEmailUpdateEvent;
use Sugarcrm\Sugarcrm\custom\Hint\Queue\Event\UserLicensedEvent;
...
class HintUsersHook extends LogicHook
{
    ...
    public function afterSave($user, $event, $arguments)
    {
        ...
        $perUserLicensing = $manager->perUserLicensing();
        ...
        if (!empty($arguments['dataChanges']['status'])) {
            ...
            if ($statusChange === $inactive && $statusPrev !== $inactive) {
                $hasLicense = array_search("HINT", json_decode($user->license_type, true));

                if ($perUserLicensing && $hasLicense !== false) {
                    $removeLicense = true;
                } else if (!$perUserLicensing) {
                    // If this is not per-user licensing, send a standard user inactivation event
                    // and return.
                    $this->eventQueue->recordEvent(new UserInactiveEvent([
                        'userId' => $user->id,
                    ]));
                    return;
                }
        ...
        if (!empty($arguments['dataChanges']['license_type']) || $removeLicense) {
            ...
            if (!$oldLicensedUser && $newLicensedUser) {
                ...
                if ($userBean->previously_licensed_c || $userAccountsetsRows) {
                    $manager->reviveAccountsetsAndTargets($user->id);
                } else {
                    \HintAccountset::createUserAccountset($user);
                }
            } else if (($oldLicensedUser && !$newLicensedUser) || $removeLicense) {
                ...
                // as previously_licensed_c (in case they become re-licensed later).
                //
