...
({
    ...
    fieldTypeTranslations: {
        ...
        'hint_account_pic_c' : 'LBL_HINT_COMPANY_LOGO',
        'hint_contact_pic_c' : 'LBL_HINT_CONTACTS_AVATAR',
...
// Delete the enriched phone numbers also in original model and fill the rest into siBean in order.
    ...
    _setFieldViewModel: function() {
        ...
        _.each(this.metadata, function(panel) {
            _.each(panel.fields, function(field) {
                ...
                if (self.moduleName !== 'Accounts' && (panel.name == 'company_info' || panel.name == 'company_extended') && ! /^hint_[\w]*_c/i.test(field.name)) {
                    if (field.type !== 'enum')
    ...
    initialize: function(options) {
        ...
        if (this._originalModel.get('picture')) {
            this._originalModel.save('hint_contact_pic_c', '')
        }
...
// encapsulate access to storage representation
    ...
    _enrichModel: function() {
        ...
        if (this.context) {
            this.shouldShowHintCloudLogo = isCreateDrawerAccountsOpen ? this.context.get('model').exitDropdownNoEnrich : true;
        }
        ...
        if (isCreateDrawerAccountsOpen) {
            if (((this.previousAccountsName !== this._originalModel.get('name')) && !_.isEmpty(this._originalModel.get('name'))) || this.cancelName) {
            ...
            } else if (((this.previousAccountsWebsite !== this._originalModel.get('website')) && !_.isEmpty(this._originalModel.get('website'))) || this.cancelName){
                _.each(accountsHintFields, function(field) {
        ...
        var SIBean = (this.moduleName === 'Leads' || this.moduleName === 'Contacts') ? JSON.stringify(this.convertSugarBeanToHintPayload(
        ...
        this.issueDataEnrichmentRequest('GET', this._stage2urlV2 + endpoint, {
            ...
            success: function (data) {
                if (self.moduleName === 'Accounts' && self.isCreateDrawerAccountsOpen) {
                    ...
                    // the dropdown and it's NOT the same as the account name from the previously fetched data (especially
                    // when the account to enrich was inferred from the website provided by the user), then we want to
                    // show the clouds. However, if the previous scenario were the same except that the previous account
                    // fetched was the same as the account we just tried to fetch now, then we don't show the cloud icons.
                    var previousAndCurrentNameEqual = data.bean.name.toLowerCase() === self.lastFetchedName.toLowerCase();
                ...
                if (self.moduleName === 'Leads') {
                    if (self.isDarkMode) {
                        self.$('.hint-preview-icon-darkmode--cloud[data-name="' + 'hint_account_website_c' + '"]').addClass('hidden');
                    } else {
    ...
    _getFieldsFromBean: function(bean) {
        return {
            ...
            'hint_account_size_c': String(bean.hint_account_size_c),
            'hint_account_location_c': bean.hint_account_location_c,
            'hint_account_industry_c': bean.hint_account_industry_c,
            'hint_account_founded_year_c': String(bean.hint_account_founded_year_c),
            ...
            'hint_account_fiscal_year_end_c': String(bean.hint_account_fiscal_year_end_c),
            'hint_account_pic_c':  bean.hint_account_logo_c
        };
    },
    ...
    _newLogosFound: function(bean, attr) {
        ...
        if (accountsSicFieldFound) {
            ...
            var doPhotoURLsDiffer = bean['hint_account_logo_c'] != this._originalModel.attributes['hint_account_pic_c'];
            ...
            var doPhotosDiffer = bean['hint_photo_c'] && bean['hint_photo_c'] != this._originalModel.attributes['hint_contact_pic_c'];
    ...
    _warningAlert: function(attr, value, msg) {
        ...
        var message = msg ? msg : SUGAR.App.lang.get('LBL_HINT_MSG_OVERWRITE_FIELD', this.module, {fieldName: fieldName});
        var accountsSicFieldFound = this._accountContainsSICLabel(attr);
        ...
        if (warningMessageDisplayFlag || doIndividualFieldsDiffer) {
            app.alert.show('message-id', {
                ...
                onConfirm: function () {
                    ...
                    const isValidAvailablePhoneField = self.isValidPhoneType(attr) && value.indexOf(self._originalModel.get(attr) + ', ') > -1;
    ...
    _warningAlertForCreateDrawer: function(attr, value, msg) {
        ...
        var recordSicViewField = accountsSicFieldFound ? this._originalModel.get('sic_code') : this._originalModel.get(attr);
    ...
    _accountContainsSICLabel: function(attr) {
        return this.moduleName === 'Accounts' && attr === 'hint_account_sic_code_label_c';
    },
    ...
    _isValueUpdated: function(attr, key) {
        if (!_.isEmpty(this._originalModel.get(key))) {
            ...
            if (key === 'title' && (this._originalModel.get('picture') !== this.model.get('hint_photo_c'))) {
                return true;
            }
    ...
    _resetOnCancel: function(attr) {
        var accountsLogoURL = this._originalModel.get('hint_account_pic_c');
        var foundNewLogo= (!_.isEmpty(accountsLogoURL) &&
            accountsLogoURL !== this.model.get('hint_account_logo_c'));

        var contactsLeadsPhotoURL = this._originalModel.get('hint_contact_pic_c');
        var contactPhotoFromHint = this.model.get('hint_photo_c');
        ...
        if (attr === 'title' && foundNewPhoto) {
            this._warningAlertForImageOverwrite('hint_contact_pic_c', true, false);
        } else if (attr === 'website' && foundNewLogo) {
            this._warningAlertForImageOverwrite('hint_account_pic_c', true, false);
        } else {
    ...
    _enrichFieldsToModules: function(attr, value) {
        ...
        if (this.moduleName === 'Accounts') {
            var accountsLogoURL = this._originalModel.get('hint_account_pic_c');
            attr['hint_account_pic_c'] = this.model.get('hint_account_logo_c');
            for (var key in attr) {
                if (attr.hasOwnProperty(key)) {
                    if (key === 'hint_account_sic_code_label_c') {
                        attr['sic_code'] = this.model.get('sic_code');
                    } else if (key !== 'hint_account_pic_c') {
                        attr[key] = String(this.model.get(key));
                    }
                }
            }
            this._originalModel.save(attr, value);
            if (!_.isEmpty(accountsLogoURL) && accountsLogoURL !== this.model.get('hint_account_logo_c')) {
                ...
                } else if (attr === 'hint_account_pic_c') {
                    this._warningAlertForImageOverwrite('hint_account_pic_c', false, false);
                }
            }
        } else {
            attr['hint_contact_pic_c'] = this.model.get('hint_photo_c');
            for (var key in attr) {
                ...
                if (isValidPhoneField) {
                    ...
                    attr[key] = this.model.get('hint_account_website_c');
                } else if (key !== 'hint_contact_pic_c') {
                    attr[key] = String(this.model.get(key));
                }
            }
            this._originalModel.save(attr, value);
            var contactsLeadsPhotoURL = this._originalModel.get('picture');
            if (!_.isEmpty(contactsLeadsPhotoURL) && (contactsLeadsPhotoURL !== this.model.get('hint_photo_c'))) {
                // attr will be an object when both the title and profile logo needs to be changed.
                if (typeof(attr) === 'object') {
                    this._saveHintsImageURL(overwriteOnlyLogo);
                } else {
                    this._warningAlertForImageOverwrite('hint_contact_pic_c', false, false);
                }
            }
        }
    },
    ...
    _setFieldsInCreateDrawer: function(attr, value) {
        if (this.moduleName === 'Accounts') {
            var accountsLogoURL = this._originalModel.get('hint_account_pic_c');
            var hintAccountLogoUrl = this.model.get('hint_account_logo_c');
            var accountsSicFieldFound = this._accountContainsSICLabel(attr);
            for (var key in attr) {
                if (attr.hasOwnProperty(key)) {
                    if (key === 'hint_account_sic_code_label_c') {
                        attr['sic_code'] = this.model.get('sic_code');
                    } else if (key === 'hint_account_logo_c') {
                        attr['hint_account_pic_c'] = hintAccountLogoUrl;
                    } else {
                        attr[key] = String(this.model.get(key));
                    }
                }
            }
            if (this._isValidAttrForLogoEnrich(attr, 'hint_account_logo_c') && (_.isEmpty(accountsLogoURL) || accountsLogoURL !== hintAccountLogoUrl)) {
            ...
            var contactsPhotoURL = this.model.get('hint_photo_c');
            for (var key in attr) {
                var isValidPhoneField = attr.hasOwnProperty(key) && this.isValidPhoneType(key);
                var phoneNumber = this.model.get(key);
                if (isValidPhoneField) {
                    attr[key] = phoneNumber && this._getTruncatedPhoneBasedOnFieldWidth(phoneNumber, key);
                } else if (key === 'hint_photo_c') {
                    attr['hint_contact_pic_c'] = contactsPhotoURL;
                }  else {
                    attr[key] = String(this.model.get(key));
                }
            }
            this._originalModel.set(attr, value);
            var contactsLeadsPhotoURL = this._originalModel.get('hint_contact_pic_c');
            if (this._isValidAttrForLogoEnrich(attr, 'hint_photo_c') && (_.isEmpty(contactsLeadsPhotoURL) || contactsLeadsPhotoURL !== contactsPhotoURL)) {
                this._saveHintsImageURL();
            }
        }
    },
...
// encapsulate access to storage representation
    ...
    _doOnlyAccountsLogosDiffer: function(para1) {
        var accountsLogoURL = this._originalModel.get('hint_account_pic_c');
        var doesLogoUrlDiffer = !_.isEmpty(accountsLogoURL) && accountsLogoURL !== this.model.get('hint_account_logo_c');
        var isWebsiteFieldSame = para1 === 'website' && (this._originalModel.get('website') === this.model.get('website'));

        return isWebsiteFieldSame && doesLogoUrlDiffer;
    },

    _doNonAccountsPhotoURLsDiffer: function() {
        var photoURL = this._originalModel.get('hint_contact_pic_c');
        var photoURLFromHint = this.model.get('hint_photo_c');
        var imagePathInSugar = this._originalModel.get('picture');
    ...
    _updateBaseModel: function(para1, para2) {
        ...
        var recordViewField = accountsSicFieldFound ? this._originalModel.get('sic_code') : this._originalModel.get(para1);
            ...
            if (this.moduleName !== 'Accounts') {
                if (msg) {
                    ...
                    if (para1 === 'title' && this._doOnlyCreateDrawerPhotosDiffer(para1)) {
                        this._warningAlertForImageOverwrite('hint_contact_pic_c', true, false);
                    } else if (para1 === 'title' && this._doNonAccountsCreatePhotoURLsDiffer() && (this._originalModel.get('title') !== this.model.get('title'))) {
                        this._warningAlertForImageOverwrite('hint_contact_pic_c', true, true);
                    } else if (para1 === 'title' && _.isEmpty(this._originalModel.get('hint_contact_pic_c'))) {
                        this._saveHintsImageURL();
                ...
                if (para1 === 'website' && _.isEmpty(this._originalModel.get('hint_account_pic_c'))) {
                    this._saveHintsImageURL();
                } else {
                    this._warningAlertForCreateDrawer(para1, para2, msg);
                }
    ...
    _saveDataInRecordField: function(para1, para2) {
        var needsAccountPic = para1 === 'website' && _.isEmpty(this._originalModel.get('hint_account_pic_c'));
        var needsContactPic = para1 === 'title' && _.isEmpty(this._originalModel.get('hint_contact_pic_c'));
        var enrichNewContactPicAndTitle = _.isEmpty(this._originalModel.get('title')) && this._doNonAccountsPhotoURLsDiffer();
    ...
    _saveHintsImageURL: function(overwriteOnlyImage) {
        ...
        var enrichAccountsFieldForLogo = {
            ...
            'hint_account_pic_c' : this.model.get('hint_account_logo_c')
        };
        ...
        var enrichNonAccountFieldForLogo = {
            'hint_contact_pic_c' : this.model.get('hint_photo_c'),
        ...
        if (this.moduleName === 'Accounts') {
            if (doEnrichWebsiteLogoDuo) {
                ...
                if (this.stage2CreateMode) {
                    this._originalModel.set('hint_account_pic_c', this.model.get('hint_account_logo_c'));
                } else {
                    this._originalModel.save('hint_account_pic_c', this.model.get('hint_account_logo_c'));
                }
            }
        } else {
            if (doEnrichTitlePhotoDuo) {
                ...
                if (this.stage2CreateMode) {
                    this._originalModel.set('picture_guid', null);
                    this._originalModel.set('hint_contact_pic_c', this.model.get('hint_photo_c'));
                } else {
                    var field = this._getSidebarImageField();
                    if (!_.isEmpty(field)) {
                        field.deleteImage(this._originalModel, this.model.get('hint_photo_c'));
                    } else {
                        this._originalModel.save('hint_contact_pic_c', this.model.get('hint_photo_c'));
                    }
                }
            }
        }
