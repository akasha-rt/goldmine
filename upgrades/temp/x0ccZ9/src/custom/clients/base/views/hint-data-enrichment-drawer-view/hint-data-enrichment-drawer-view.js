...
({
    ...
    initialize: function(view) {
        ...
        this.moduleList = SUGAR.App.metadata.getFullModuleList();
        this.shouldShowAccountsTab = this.moduleList['Accounts'] ? true : false;
        this.shouldShowContactsTab = this.moduleList['Contacts'] ? true : false;
        this.shouldShowLeadsTab = this.moduleList['Leads'] ? true : false;
    ...
    restoreDefaults: function(event) {
        ...
        app.alert.show('message-id', {
            level: 'confirmation',
            messages: SUGAR.App.lang.get('LBL_HINT_CONFIG_WARNING_MESSAGE', self.currentModule, {module: self.currentModule}),
            autoClose: false,
            onConfirm: function () {
                self._restoreModulesMetaData();
                self.render();
                app.events.trigger('hint:config:defaults:restored', { module: self.currentModule, metadata: self._getMetadataForModule(self.currentModule)});
            },
            onCancel: function () {
                $.noop();
            }
