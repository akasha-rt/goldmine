...
({
	...
	initialize: function(options) {
		...
		this.sugarPROFlavor = SUGAR.App.metadata.getServerInfo().flavor === 'PRO';
		this.licensedHint = _.contains(app.user.attributes.licenses, 'HINT');
		this.proNonLicensedUser = !this.licensedHint && this.sugarPROFlavor && app.hint.versionCompare('10.3.0', true) >= 0;

		this._super('initialize', [options]);
		this._moduleName = this.context.get('module');
		this._detectAttrs_Contacts = ['first_name', 'last_name'];
		this._detectAttrs_Leads = ['first_name', 'last_name', 'account_name', 'website', 'title', 'phone_work'];
		...
		if (this._moduleName === 'Accounts') {
			...
			this._defaultMessage1 = app.lang.get('LBL_HINT_HELP_DEFAULT_MESSAGE1B', this._moduleName, {currentModule: this._moduleSingular});
			this._defaultMessage2 = '';
		}
