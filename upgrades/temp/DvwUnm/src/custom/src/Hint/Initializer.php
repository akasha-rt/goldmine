...
use Sugarcrm\Sugarcrm\custom\Hint\Queue\Event\InstanceInitCloneEvent;
...
use Sugarcrm\Sugarcrm\custom\Hint\Queue\Event\UpdateLicenseEvent;
...
class Initializer implements LoggerAwareInterface
{
    ...
    public function init()
    {
        ...
        // If HintSeatsJob has not run (fresh installation), then we defer the creation of
        // accountsets/targets to the execution of the HintSeatsJob on sugar 10.3+ and
        // hint 5.4.0+ for per-user licensing management. This includes the
        // InstanceInitCompletedEvent, which is fired at the end of the
        // HintSeatsJob on successful license assignments.
        //
        // Otherwise, in the case of an upgrade (where the HintSeatsJob will not run again), then
        // proceed as normal, sending the correct targets for licensed users only (in the case
        // of hint 5.4.0+ sugar 10.3+).
        $manager = Manager::instance();
        $hintSeatsFlag = ConfigurationManager::getHintConfigEntry(HintConstants::HINT_CONFIG_SEATS);
        if ($manager->perUserLicensing() && !$hintSeatsFlag['value']) {
            return;
        }
            ...
            while (true) {
                ...
                if ($manager->perUserLicensing()) {
                    ...
                    foreach ($userRows as $userRow) {
                        if (strpos($userRow['license_type'], '"HINT"') !== false) {
                            $users[] = $userRow;
                        }
                    }
                } else {
                ...
                foreach ($rows as $row) {
                    ...
                    $data[] = array_merge($row, [
                        ...
                        'credentials' => $this->getCorrectSiteURL($row['credentials'], true),
                    ]);
                }
            ...
            while (true) {
                ...
                if ($manager->perUserLicensing()) {
                    ...
                    foreach ($accountsetRows as $accountset) {
                        ...
                        $hasHintLicense = strpos($userIdLicenseMap[$accountset['user_id']], '"HINT"');
                        if ($hasHintLicense !== false) {
                            $rows[] = $accountset;
                        }
                    }
                } else {
     ...
     * @param $targetEntry credential triple; an object with three keys email, timezone and siteUrl.
    ...
    private function setDEFieldConfig() {
        ...
        $hintApiCalls->registerConfigToEnrichBeanEndpoint($hintApiCalls->privilegeToken,
            json_decode($configDataBeanData['config_data']));
    }
