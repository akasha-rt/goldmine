...
({
    ...
    initialize: function(view) {
        ...
        this.bucketList = {}; // It eventually holds the enrich Fields Data from the configDB.
        ...
        this.fieldConfigCache = {
            ...
            _maxLifetime: 5*60*1000,
            _lastUpdated: SUGAR.App.user.get('fieldConfigCacheLastUpdated'),
            _value: SUGAR.App.user.get('fieldConfigCache'),
            update: function (config) {
                SUGAR.App.user.set('fieldConfigCache', config);
                this._value = config;
                var lastUpdate = Date.now();
                SUGAR.App.user.set('fieldConfigCacheLastUpdated', lastUpdate);
            ...
            get: function (callback) {
                ...
                if (!this._lastUpdated || Date.now() - this._lastUpdated >= this._maxLifetime) {
                    var cache = this;
                    self._callStage2API('read', 'hint/enrich/config', {
                        success: function(data) {
                            var res = data.response;
                            var config = {id: res.id, data: JSON.parse(res.config_data)};
                            cache.update(config);
                    ...
                    // we have value; retrieve value and populate bucketList and configId
                    // no need to render
                    callback(this._value, false);
                }
            }
        };
        this._populateEnrichOptions();// Let the UI render irrespective of token and call the token endpoint only on save.
    ...
    _saveConfig: function() {
        ...
        self._withValidAuthenticationToken(function() {
            ...
            if (!_.isEmpty(self.bucketList)) {
                ...
                self.fieldConfigCache.update({id: self.configId, data: self.bucketList});
                self._callStage2API('update', 'hint/enrich/config', {
                    'id': self.configId,
                    'config_data': self.bucketList
                });
            }
    ...
    _callToSendEnrichFields: function(method, dataEnrichmentAccessToken) {
        ...
        if (!_.isEmpty(dataEnrichmentAccessToken)) {
            var URL = SUGAR.App.user.get('dataEnrichmentUrl') + '/config-fields-for-enrich';
            var data = {
            ...
            var apiOptions = {
                headers : {
                    'authToken' : dataEnrichmentAccessToken
                }
            };
            ...
            app.api.call(method, URL, data, {
                ...
                error: function (err) {
                    app.logger.error('Failed to reach ' + URL + JSON.stringify(err));
                }
            }, apiOptions);
        }
        return;
    ...
    _populateEnrichOptions: function() {
        var self = this;
        self.fieldConfigCache.get(function(config, render) {
            self.bucketList = config.data;
    ...
    resetEnrichFieldsView: function() {
        var self = this;
        _.each(['Accounts', 'Contacts', 'Leads'], function(moduleName) {
    ...
    removeUnwantedfield: function(module) {
        var disabledFields = {
            'Accounts': ['name', 'website', 'hint_account_logo_c', 'picture'],
            'Contacts': ['full_name', 'email', 'hint_photo_c', 'account_name', 'name', 'hint_account_logo_c', 'picture', 'hint_education_2_c'],
            'Leads': ['full_name', 'email', 'hint_photo_c', 'hint_account_logo_c', 'name', 'picture', 'hint_education_2_c']
        };
    ...
    isFieldDisabled: function(module, field) {
        var disabledFields = {
            'Accounts': [],
            'Contacts': ['full_name', 'hint_photo_c', 'account_name', 'picture', 'hint_education_2_c'],
            'Leads': ['full_name', 'hint_photo_c', 'picture', 'hint_education_2_c']
        };
