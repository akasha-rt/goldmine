...
class UploadFile
{
	...
	function final_move($bean_id, $temporary = false)
	{
        ...
        if($this->use_soap) {
            ...
            $fileType = $_FILES[$this->field_name]['type'] ?? null;
            //skip convert if filetype is image or video to save file quality
            $convert = !in_array(substr($fileType, 0, 6), ['image/', 'video/']);
            if (!UploadStream::move_uploaded_file($_FILES[$this->field_name]['tmp_name'], $destination, $convert)) {
...
class UploadStream
{
     ...
     * @param bool $convert Should file be encoded
     */
    public static function move_uploaded_file($upload, $path, $convert = true)
    {
        $to = self::path($path);
