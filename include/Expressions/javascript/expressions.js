...
return new SUGAR.ConstantExpression(parseFloat(fixed));if((/^"[\s\S]*"$/).exec(expr)!=null){expr=expr.substring(1,expr.length-1);return new SUGAR.StringLiteralExpression(expr);}
if(expr=='')
...
{var SE=SUGAR.expressions;var ts=",",ds=".";if(SE.userPrefs){if(SE.userPrefs.num_grp_sep)
ts=SE.userPrefs.num_grp_sep;if(SE.userPrefs.dec_sep)
ds=SE.userPrefs.dec_sep;};num=SE.replaceAll(num,ts,"");num=SE.replaceAll(num,ds,".");}
