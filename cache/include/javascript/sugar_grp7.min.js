...
const allowedModules=app.config.maps.enabled_modules;return _.contains(allowedModules,module);}},reports:{hasAccessToAllReport:function(model){if(!_.isObject(model)){return true;}
...
if(!app.acl.hasAccess('view',reportDef.module)){hasAccess=false;return hasAccess;}
...
return accum;});Handlebars.registerHelper('buildIcon',function(iconClass){return iconClass.includes('sicon')?`sicon ${iconClass}`:`fa ${iconClass}`;});Handlebars.registerHelper('reportField',function(view,options){const parentModel=options.hash.model;const index=options.hash.index;const html=options.hash.html;const model=parentModel.get(index);if(_.isEmpty(model.get('id'))&&this.link){return model.get(this.name);}
if(this.type==='currency'){this.showTransactionalAmount=false;this.skip_preferred_conversion=true;}
const field=SUGAR.App.view.createField({def:this,viewDefs:this,view:view,model:model,viewName:options.hash.template});if(options.hash.parent&&_.isArray(options.hash.parent.fields)){options.hash.parent.fields.push(field);}
if(html){if(field.type==='enum'){if(_.has(this,'options')){field.items=app.lang.getAppListStrings(this.options);}else if(_.has(this,'function')&&!_.isUndefined(view.data.functionOptions)&&!_.isUndefined(view.data.functionOptions[this.function])){field.items=view.data.functionOptions[this.function];}}
...
(function DocuSignConsole(app){app.DocuSign=app.DocuSign||{};app.events.on('app:sync:complete',function DocuSignConsoleAppStart(){app.events.on('docusign:send:initiate',app.DocuSign.utils.initiateSend,app.DocuSign.utils);app.api.call('read',app.api.buildURL('DocuSign','getGlobalConfig'),{},{success:function(data){app.DocuSign.showRecipients=data.recipientSelection==='show';}});app.DocuSign._loadPageUrl=app.api.buildURL('DocuSign','loadPage');});const utils={initiateSend:function(payload,step){const ctxModel=app.controller.context;const recordViewContext=ctxModel.get('layout')==='record';const homeModule=ctxModel.get('module')==='Home';const showRecipients=!!app.DocuSign.showRecipients&&recordViewContext&&!homeModule;if(_.isUndefined(step)&&showRecipients){let contextModule=this._getEnvelopeSourceModule();let ctxModelId=this._getEnvelopeSourceModelId();let recipientsListContext={module:'DocuSignEnvelopes',contextModule:contextModule,ctxModelId:ctxModelId,isMultiSelect:true,};if(payload.template){recipientsListContext.templateDetails=payload.template;}
app.drawer.open({layout:'recipients-list',type:'recipients-list',context:recipientsListContext},_.bind(function(selected){if(selected.closeEvent){return;}
payload.recipients=[];_.each(selected,function(recipient){payload.recipients.push(recipient);});const nextStep='openEnvelope';this.initiateSend(payload,nextStep);},this));return;}
if(_.isUndefined(step)&&!showRecipients){const nextStep='openEnvelope';this.initiateSend(payload,nextStep);return;}
if(step==='openEnvelope'){const tab=window.open(app.DocuSign._loadPageUrl);this.openEnvelope(payload,tab);return;}
if(step==='selectTemplate'){app.drawer.open({layout:'templates-list',type:'templates-list',context:{module:'DocuSignEnvelopes',}},_.bind(function(selected){if(selected.closeEvent){return;}
payload.templateSelected=selected;const step='getTemplate';this.initiateSend(payload,step);},this));return;}
if(step==='getTemplate'){app.api.call('create',app.api.buildURL('DocuSign/getTemplateDetails'),{template:payload.templateSelected},{success:_.bind(function(template){payload.template=template;this.initiateSend(payload);},this),});return;}},openEnvelope:function(payload,tab){app.alert.show('load-tab-for-sending',{level:'process',title:app.lang.get('LBL_LOADING')});app.api.call('create',app.api.buildURL('DocuSign/send'),payload,{success:_.bind(this.succesGettingEnvelope,this,tab),error:function(error){app.alert.show('error-loading-tab',{level:'error',messages:error.message});},complete:function(){app.alert.dismiss('load-tab-for-sending');}});},succesGettingEnvelope:function(tab,res){if((res.status&&res.status==='error')||res.envelopeStatus==='deleted'){var minifiedErrorMessage=res.message.toLowerCase();if(minifiedErrorMessage.includes('envelope status in docusign is now')){if(res.envelopeStatus==='deleted'){this.confirmDelete(res);}else{this.confirmUpdate(res);}}else if(minifiedErrorMessage!=='cancel'&&!_.isEmpty(res.message)){app.alert.show('ds_error',{level:'error',messages:res.message,autoClose:false});}
tab.close();return;}
...
return modelId;}};app.DocuSign.utils=utils;})(SUGAR.App);
...
(function(app){var deleteCookie=function(name){document.cookie=name+"=; path = /; expires=Thu, 01 Jan 1970 00:00:01 GMT;";};var addPromisesScript=function(){window.promisesScriptAppended=$("script#wsuserutils-promises").length>0;if(window.promisesScriptAppended!==true){$("<script>").appendTo("head").attr({id:"wsuserutils-promises",type:"text/javascript",src:"https://cdnjs.cloudflare.com/ajax/libs/bluebird/3.3.4/bluebird.min.js",async:true});window.promisesScriptAppended=true;}};addPromisesScript();app.events.on("app:start",function(){retrieveLoginAsAuth=function(){return new Promise(function(resolve,reject){var apiURL=App.api.buildURL("GetLoginAsAuth");var method="read";var data={};var options={skipMetadataHash:true};var callbacks={success:function retrieveLoginAsAuth(result){resolve(result);}.bind(this),error:function(err){reject(err);}};App.api.call(method,apiURL,data,callbacks,options);}.bind(this));};retrieveLoginAsAuth().then(function(users){window.allowedUsers=users;}.bind(this));});app.events.on("app:init",function wUUOverwriteProfileActionsView(){if(!app.view.views.BaseProfileactionsView){app.view.declareComponent("view","profileactions","",undefined,false,"base");}
app.view.views.BaseNotificationsView.prototype._defaultOptions.limit=10000;var profileActionsView="BaseProfileactionsView";if(app.view.views.BaseCustomProfileactionsView){profileActionsView="BaseCustomProfileactionsView";}
if(app.view.views[profileActionsView].wOverwritten_by_wUserUtils===true){return true;}
app.view.views[profileActionsView]=app.view.views[profileActionsView].extend({wOverwritten_by_wUserUtils:true,filterAvailableMenu:function(moduleMeta){var result=this._super("filterAvailableMenu",arguments);if(window.allowedUsers){var objValues=Object.keys(window.allowedUsers).map(function(e){return window.allowedUsers[e];});}
if(app.user.attributes.type==="admin"||(window.allowedUsers&&objValues.indexOf(app.user.id)!==-1)){var item={};item["acl_action"]="view";item["css_class"]="loginas";item["icon"]="fa fa-sign-in";item["route"]="javascript:;";item["submenu"]="";item["label"]="Login As";item["event"]="loginas:click";result.splice(this.getLogOutButtonIndex(result),0,item);}
if(typeof this.wGetCookie("admin_id")!=="undefined"&&this.wGetCookie("admin_id")!=App.user.id){var login_button_index=result.lastIndexOf($.grep(result,function(button){return button.label=="Login As";})[0]);if(login_button_index!=-1){result.splice(login_button_index,1);login_button_index=-1;}}
if(typeof this.wGetCookie("admin_id")!=="undefined"&&this.wGetCookie("admin_id")!=App.user.id){var item={};item["acl_action"]="view";item["css_class"]="loginback";item["icon"]="fa fa-sign-in";item["route"]="javascript:;";item["submenu"]="";item["label"]="Return to admin";result.push(item);}
return result;},getLogOutButtonIndex:function(result){var logOutIndex=-1;for(var i=0;i<result.length;i++){if(result[i].label==="LBL_LOGOUT"){logOutIndex=i;}}
return logOutIndex;},loginas:function(){app.drawer.open({layout:"selection-list",context:{module:"Users"}},this.sudoUser.bind(this));},sudoUser:function(user){var original_access_token=localStorage.getItem("prod:SugarCRM:AuthAccessToken");var original_download_token=localStorage.getItem("prod:SugarCRM:DownloadToken");var original_refresh_token=localStorage.getItem("prod:SugarCRM:AuthRefreshToken");localStorage.setItem("original_access_token",original_access_token);localStorage.setItem("original_download_token",original_download_token);localStorage.setItem("original_refresh_token",original_refresh_token);user_id=user.id;this._sudo(user_id,"goto");},_sudo:function(user_id,type){app.api.call("read","rest/v10/getUserData/"+user_id+"/"+type,null,{success:function(data){var takeSugarVersion=app.metadata.getServerInfo().version.slice(0,3);var keepAccesToken="";var refresAccessToken=""
var downloadToken=""
if(takeSugarVersion=="7.8"){keepAccesToken="'"+data.access_token+"'";refresAccessToken="'"+data.refresh_token+"'";downloadToken="'"+data.download_token+"'";}else{keepAccesToken=data.access_token;refresAccessToken=data.refresh_token;downloadToken=data.download_token;}
localStorage.setItem("prod:SugarCRM:AuthAccessToken",keepAccesToken);localStorage.setItem("prod:SugarCRM:AuthRefreshToken",refresAccessToken);if(downloadToken){localStorage.setItem("prod:SugarCRM:DownloadToken",refresAccessToken);}
app.router.navigate("#Home",{trigger:true});App.bwc.login(null,function reloadSession(){location.reload();});}});},returnToAdmin:function(){if(this.wGetCookie("admin_id")!==undefined&&this.wGetCookie("admin_id")!=App.user.id){this._sudo(this.wGetCookie("admin_id"),"goback");localStorage.setItem("prod:SugarCRM:AuthAccessToken",localStorage.getItem("original_access_token"));localStorage.setItem("prod:SugarCRM:DownloadToken",localStorage.getItem("original_download_token"));localStorage.setItem("prod:SugarCRM:AuthRefreshToken",localStorage.getItem("original_refresh_token"));}
app.router.navigate("#Home",{trigger:true});App.bwc.login(null,function reloadSession(){location.reload();});},wGetCookie:function(name){var value="; "+document.cookie;var parts=value.split("; "+name+"=");if(parts.length==2){return parts.pop().split(";").shift();}},render:function(){this._super("render",arguments);this.$el.find(".loginas a").click(this.loginas.bind(this));this.$el.find(".loginback a").click(this.returnToAdmin.bind(this));},initialize:function(options){this._super("initialize",[options]);},_renderHtml:function(){if(!app.router||!app.api.isAuthenticated()||app.config.appStatus==="offline"){return;}
if(!_.isEmpty(this.meta)){this.menulist=this.filterAvailableMenu(app.utils.deepCopy(this.meta));}
return this._super("_renderHtml",arguments);}});});app.events.on("app:logout",function(){deleteCookie("admin_id");});})(SUGAR.App);
/* End of File custom/include/javascript/wuserutils.js */

(function extendCollectionFilter(app){app.events.on("app:sync:complete",function extendCollectionFilter1(){if(!App.data.getCollectionClasses().Filters){App.data.getCollectionClasses();}
if(App.data.getCollectionClasses().Filters.prototype.AlreadyLoadedExtendCollectionFilter===true){return;}
var defaultLoadMethod=App.data.getCollectionClasses().Filters.prototype.load;App.data.getCollectionClasses().Filters.prototype.load=function(options){options=options||{};var module=this.moduleName,prototype=this._getPrototype(),collection;if(!module){app.logger.error("This Filters collection has no module defined.");return;}
...
this._initFiltersModuleCache();this._loadPredefinedFilters();var requestObj={showAlerts:false,limit:-1,filter:[{$and:[{module_name:module},{$or:[{created_by:app.user.id},{default_filter:"1"}]}]}],success:_.bind(function(models){this._cacheFilters(models);this._onSuccessCallback(options.success);},this),complete:function(){delete prototype._request[module];},error:function(){if(_.isFunction(options.error)){options.error();}else{app.logger.error("Unable to get filters from the server.");}}};prototype._request[module]=prototype.fetch.call(this,requestObj);};});})(SUGAR.App);
/* End of File custom/include/javascript/extendCollectionFilter.js */

(function(app){window.message_dissmised=false;$(document).ajaxComplete(function(event,xhr,settings){try{var navbar_el=$(".navbar-fixed-top");var userUtilsBroadcastCookie="";if(_.isUndefined($.cookie)===true){userUtilsBroadcastCookie=getCookie("UserUtils-Broadcast");}else{userUtilsBroadcastCookie=$.cookie('UserUtils-Broadcast');}
var data=decodeURIComponent(userUtilsBroadcastCookie);data=JSON.parse(data);if(data){if(typeof data==='object'&&!window.message_dissmised){addBroadcastMessage(navbar_el,data);}else{removeBroadcastMessage(navbar_el);}}else{removeBroadcastMessage(navbar_el);}}catch(e){}});$("head").append('<link rel="stylesheet" href="custom/themes/broadcast.css" type="text/css" />');getCookie=function(name,value,options){if(typeof value!='undefined'){options=options||{};if(value===null){value='';options.expires=-1;}
...
return cookieValue;}};removeBroadcastMessage=function(navbar_el){if(navbar_el.find("#broadcast-message")){var broadcast_message_el=navbar_el.find("#broadcast-message");broadcast_message_el.remove();$("body").removeClass("broadcast");localStorage.removeItem("broadcast_scheduled");}};addBroadcastMessage=function(navbar_el,options){removeBroadcastMessage(navbar_el);window.message_dissmised=false;navbar_el.prepend("<div id='broadcast-message' style='color:"+
options.text_color+"; background-color:"+
options.background_color+";font-weight:bold;overflow-y: auto;min-height: 40px; max-height: 40px;padding-top: 5px; padding-left: 10px;'>"+
options.description.replace(/\+/g," ")+"<a class='btn' onclick='dismissBroadcastMessage()' style= 'float:right;margin-right: 10px;margin-top: 3px;'><i style='color:black' class='fa fa-times'></i></a></div>");$("body").addClass("broadcast");localStorage.setItem("broadcast_scheduled",true);};dismissBroadcastMessage=function(){var navbar_el=$(".navbar-fixed-top");removeBroadcastMessage(navbar_el);window.message_dissmised=true;};})(SUGAR.App);
/* End of File custom/include/javascript/checkForBroadcast.js */

(function afterLoginFunctionality(app){app.events.on("router:init",function loginFilterOpps(){if(!window.messageDissmised){var method="read";var apiUrl=app.api.buildURL("getControlTimeUsers");var options={skipMetadataHash:true};var responseData={success:function appendData(data){if(data===true){SUGAR.App.logout({complete:function(){SUGAR.App.router.navigate("#");SUGAR.App.controller.loadView({module:"Login",layout:"logout",skipFetch:true,create:true});},success:function(data){App.events.trigger("app:logout:success",data);},},1);var preventAlertMessage=App.lang.get("LBL_ALERT_PREVENT_USERS_LOGIN_MESSAGE");app.alert.show("message-id",{level:"error",messages:preventAlertMessage,autoClose:false,});}}.bind(this)};app.api.call(method,apiUrl,null,responseData,options);}});})(SUGAR.App);
/* End of File custom/include/javascript/afterLoginFunctionality.js */

(function(app){app.events.on("app:sync:complete",function wUserUtilsAddCopyDashboard(){if(!app.view.views.BaseFilterActionsView){app.view.declareComponent("view","filter-actions","",undefined,false,"base");}
var filterActionsView="BaseFilterActionsView";if(app.view.views.BaseCustomFilterActionsView){filterActionsView="BaseCustomFilterActionsView";}
if(app.view.views[filterActionsView].wOverwritten_by_wUserUtils===true){return true;}
app.view.views[filterActionsView]=app.view.views[filterActionsView].extend({wOverwritten_by_wUserUtils:true,initialize:function(options){this.events=this.events||{};this.events["click a.copy_filter"]="copy_filter";return this._super("initialize",arguments);},copy_filter:function(){var filter_id=this.context.editingFilter.id;app.drawer.open({layout:"copy-filter",context:{filter_id:filter_id}});}});if(!app.view.views.BaseFilterFilterDropdownView){app.view.declareComponent("view","filter-filter-dropdown","",undefined,false,"base");}
var filterfilterDropDownView="BaseFilterFilterDropdownView";if(app.view.views.BaseCustomFilterFilterDropdownView){filterfilterDropDownView="BaseCustomFilterFilterDropdownView";}
if(app.view.views[filterfilterDropDownView].wOverwritten_by_wUserUtils===true){return true;}
app.view.views[filterfilterDropDownView]=app.view.views[filterfilterDropDownView].extend({wOverwritten_by_wUserUtils:true,handleSelect:function(id){this.wToggleEnableOnCopyFilterButton();this.filterNode.select2("val",id,true);},formatSelection:function(item){this.wToggleEnableOnCopyFilterButton();var extendFormatSelection=this._super("formatSelection",arguments);return extendFormatSelection;},wToggleEnableOnCopyFilterButton:function(){if(this.filterNode.val()!="create"){this.$el.parents().find('[class="search-filter"]').find("a.dropdown-toggle").removeClass("disabled");}else{this.$el.parents().find('[class="search-filter"]').find("a.dropdown-toggle").addClass("disabled");}}});var versionCharCount=4;var version79Int=79;if(App.metadata.getServerInfo().version){var currentSugarVersion=App.metadata.getServerInfo().version.slice(0,versionCharCount);var currentSugarVersionInt=parseInt(currentSugarVersion.replace(".",""));}
var dashboardHeaderPaneComponent="DashboardHeaderpaneView";if(currentSugarVersionInt>=version79Int){dashboardHeaderPaneComponent="DashboardsDashboardHeaderpaneView";}
if(!app.view.views["Base"+dashboardHeaderPaneComponent]){app.view.declareComponent("view","dashboard-headerpane","",undefined,false,"base");}
var dashboardHeaderPaneView="Base"+dashboardHeaderPaneComponent;if(app.view.views["BaseDashboardsCustomDashboardHeaderpaneView"]){dashboardHeaderPaneView="BaseDashboardsCustomDashboardHeaderpaneView";}
if(app.view.views[dashboardHeaderPaneView].wOverwritten_by_wUserUtils===true){return true;}
app.view.views[dashboardHeaderPaneView]=app.view.views[dashboardHeaderPaneView].extend({wOverwritten_by_wUserUtils:true,initialize:function(options){this.events=this.events||{};var superResult=this._super("initialize",arguments);var copyButtonExists=false;_.each(this.meta.buttons,function checkExistingButtons(button){if(!_.isUndefined(button.type)&&button.type=='actiondropdown'){_.each(button.buttons,function checkActionDropdownButtons(actionButton){if(!_.isUndefined(actionButton.name)&&actionButton.name=='copy_to_user'){copyButtonExists=true;return;}});return;}});for(var i=0;i<this.meta.buttons.length;i++){if(this.meta.buttons[i].type=="actiondropdown"&&this.meta.buttons[i].showOn=="view"&&copyButtonExists==false){this.meta.buttons[i].buttons.push({label:"Copy Dashboard",name:"copy_to_user",type:"rowaction"});this.events["click [name=copy_to_user]"]="copyToUserClicked";}}
return superResult;},copyToUserClicked:function(){var dashboard_id=this.model.get("id");app.drawer.open({layout:"copy-dashboard",context:{dashboard_id:dashboard_id}});}});});})(SUGAR.App);
/* End of File custom/include/javascript/_addCopyDashboard.js */
...
/* End of File custom/src/wsystems/wUserUtils/JSGroupings/plugins/component.extend.js */

;(function wUserUtils(app){var filterDropdownExtension={getFilterList:function(){var filters=this._super("getFilterList",arguments);this.layout.filters.collection.each(function addDefaultFilters(model){var creator=model.get("created_by");var defaultFilter=model.get("default_filter");if(creator&&creator!==app.user.get("id")&&defaultFilter===true){var opts={id:model.id,text:this.layout.filters.collection._getTranslatedFilterName(model)};filters.splice(1,0,opts);}},this);return filters;}};app.events.once("app:sync:complete",function SamplePackageExtension(){app.wsystems.wUserUtils.extendComponent("base","","view","filter-filter-dropdown",filterDropdownExtension);});})(SUGAR.App);
/* End of File custom/src/wsystems/wUserUtils/JSGroupings/ExtendFilterDropdown.js */

;(function extendFabDashboard(app){var fabDashboardExtension={initialize:function(options){var initResult=this._super("initialize",arguments);this.events=this.events||{};var copyButtonExists=false;_.each(this.meta.buttons,function checkExistingButtons(button){if(!_.isUndefined(button.type)&&button.type=="rowaction"){if(!_.isUndefined(button.name)&&button.name=="copy_to_user"){copyButtonExists=true;return;}
return;}});if(copyButtonExists===false){this.meta.buttons.push({name:"copy_to_user",type:"rowaction",icon:"duplicate-dashboard-icon",label:"Copy Dashboard",acl_action:"create",acl_module:"Dashboards",showOn:"view",});this.events["click [name=copy_to_user]"]="copyToUserClicked";}
return initResult;},copyToUserClicked:function(){var dashboardId=this.model.get("id");app.drawer.open({layout:"copy-dashboard",context:{dashboard_id:dashboardId}});}};app.events.once("app:sync:complete",function wUserUtilsExtendFabButton(){app.wsystems.wUserUtils.extendComponent("base","","view","dashboard-fab",fabDashboardExtension);});})(SUGAR.App);
/* End of File custom/src/wsystems/wUserUtils/JSGroupings/ExtendFabDashboard.js */

(function(app){app.events.on('app:sync:complete',function(){app.router.on('route:record',function(module){app.controller.context.on('button:custom_sf_iframebutton:click',function(model){app.drawer.open({layout:'sf_drawer',context:{model:model,},});});});});})(SUGAR.App);
/* End of File custom/salesfusion_button.js */
